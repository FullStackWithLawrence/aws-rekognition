...
#------------------------------------------------------------------------------
locals {
  layer_slug              = "rekognition"
  layer_name              = "${local.layer_slug}_layer"
...
###############################################################################
resource "null_resource" "package_layer_rekognition" {
  triggers = {
...
resource "aws_lambda_layer_version" "rekognition" {
  ...
  depends_on = [
    null_resource.package_layer_rekognition
  ]
}
