...
resource "aws_lambda_permission" "info" {
  ...
  source_arn = "arn:aws:execute-api:${var.aws_region}:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.rekognition.id}/*/${aws_api_gateway_method.info.http_method}${aws_api_gateway_resource.info.path}"
